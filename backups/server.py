...
from modules.mouse_tracker import InputController
...
# --- INPUT TRACKING ---
last_pointer_pos = {"x": 0, "y": 0, "norm": False}
window_offset = {"x": 0, "y": 0, "w": 2560, "h": 1440}

def on_pointer_update(x, y, normalized=False, mode="absolute"):
    global last_pointer_pos
    last_pointer_pos = {"x": x, "y": y, "norm": normalized, "mode": mode}
    socketio.emit('pointer_update', last_pointer_pos)

input_ctrl = InputController(on_pointer_update)

@socketio.on('toggle_pointer_mode')
def handle_toggle_pointer(data):
    enable = data.get('enable', False)
    mode = data.get('mode', 'mouse') # 'mouse' or 'axis'
    
    if enable:
        axis_cfg = data.get('axis_config', None)
        input_ctrl.start_tracking(mode, axis_cfg)
        print(f"🖱️ POINTER: ON ({mode})")
    else:
        input_ctrl.stop_tracking()
        print("🖱️ POINTER: OFF")
    
    socketio.emit('pointer_mode_status', {'active': enable, 'mode': mode})

@socketio.on('pointer_mode_status') # Just in case client sends it back? No.
@socketio.on('update_window_pos')
def handle_window_pos(data):
    # Store global offset for Absolute Mouse Mode
    global window_offset
    window_offset = {
        'x': int(data.get('x', 0)), 
        'y': int(data.get('y', 0)),
        'w': int(data.get('w', 2560)),
        'h': int(data.get('h', 1440))
    }
    # print(f"🪟 Window moved to: {window_offset}")
...
@socketio.on('interact_map_down')
def handle_map_interaction_down(data=None):
    if not input_ctrl.running: return
    
    # 1. Map Interaction (Virtual Pointer)
    if input_ctrl.active_mode == 'pointer':
        if not input_ctrl.virtual_pointer_active:
            socketio.emit('pointer_mode_status', {'active': True})
            input_ctrl.virtual_pointer_active = True
            
        nx, ny = input_ctrl.get_cursor_pos()
        payload = {'x': 0, 'y': 0}
            
        if input_ctrl.mode in ["axis", "keys", "relative"]:
            payload = {'x': nx, 'y': ny, 'norm': True}
        else:
            if last_pointer_pos['mode'] == 'absolute':
                 rx = last_pointer_pos['x'] - window_offset['x']
                 ry = last_pointer_pos['y'] - window_offset['y']
                 if rx < 0 or ry < 0 or rx > window_offset['w'] or ry > window_offset['h']:
                     pass # Out of bounds
                 else:
                     payload = {'x': rx, 'y': ry, 'norm': False}
            else:
                 payload = {'x': nx, 'y': ny, 'norm': True}
        
        socketio.emit('simulate_pointer_down', payload)

@socketio.on('interact_cockpit_down')
def handle_cockpit_interaction_down(data=None):
    if not input_ctrl.running: return

    # 2. Cockpit Interaction
    if input_ctrl.active_mode == 'mouse' and clickable_mode_enabled and last_known_telemetry:
        nx, ny = input_ctrl.get_cursor_pos()
        _process_3d_click(nx, ny)

@socketio.on('interact_map_up')
def handle_map_interaction_up(data=None):
    if not input_ctrl.running: return
    if input_ctrl.active_mode == 'pointer':
        socketio.emit('simulate_pointer_up')

@socketio.on('interact_cockpit_up')
def handle_cockpit_interaction_up(data=None):
    pass



def _process_3d_click(global_x, global_y):
    t = last_known_telemetry
    points = load_json_file(CLICKABLE_FILE, [])
    ...
    mouse_local = (global_x - win_x, global_y - win_y)
...
@app.route('/api/pointer/toggle', methods=['POST'])
def api_toggle_pointer():
    data = request.json
    handle_toggle_pointer(data)
    return jsonify({"status": "ok", "mode": data.get("mode", "mouse")})

@app.route('/api/pointer/click', methods=['POST'])
def api_pointer_click():
    handle_interaction()
