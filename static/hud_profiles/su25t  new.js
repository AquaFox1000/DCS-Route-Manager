...
window.registerHudProfile('su25t', {
    config: {
    ...
    draw: function(ctx, data, vw, vh) {
        ...
        // DCS Modes: 6=A-A, 7=A-G (Simple Mapping)
        if (T.mode === 6) { modeStr = "A-A"; isCombatMode = true; }
        if (T.mode === 7) { modeStr = "A-G"; isCombatMode = true; }
        
        // VISIBILITY RULE:
        // 1. If Combat Mode (A-A/A-G) -> ALWAYS SHOW
        // 2. If NAV Mode -> Show ONLY if Route is Active
        ...
        if (N.hasRoute) {
            ...
            // --- THE FIX ---
            // OLD WAY (Bad): Re-calculating error using raw T.roll
            // let bankErr = (N.req_bank || 0) - T.roll;
            
            // NEW WAY (Good): Trusting the Python "Smart Error"
            // N.fd_bank already has the Input Clamp applied!
            // N.fd_pitch is also pre-calculated in Python.
            
            let pitchErr = N.fd_pitch || 0;
            let bankErr  = N.fd_bank  || 0;  // <--- This uses the Python math

            // Visual Scaling (Keep this the same)
        ...
        if (typeof Utils !== 'undefined' && data.camera && N.hasRoute) {
            
            const targetObj = { lat: N.lat, lon: N.lon, alt: N.alt || 0 };
            if (data.settings.fov) Utils.FOV_DEG = data.settings.fov;
            
            const screenPos = Utils.projectWorldToScreen(data.camera, targetObj, T, vw, vh);
            const limitX = vw * 0.3; 
