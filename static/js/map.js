...
layers['darkstreet'] = L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png', { maxZoom: 20 }); // Fallback
layers['vfr'] = L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { maxZoom: 19 }); // Placeholder
layers['vfr_night'] = L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png', { maxZoom: 19 }); // Placeholder
...
function setMapLayer(name, skipSave = false) {
    ...
    document.querySelectorAll('.menu-item span').forEach(el => {
        if (el.parentElement.getAttribute('onclick') && el.parentElement.getAttribute('onclick').includes(name)) {
...
socket.on('map_settings_update', function (data) {
    ...
    if (data.visuals) {
        ...
        const safeSet = (id, val) => { const el = document.getElementById(id); if (el) el.value = val * 100; };
    ...
    try {
        ...
        if (data.visuals) {
            ...
            if (typeof filterSettings !== 'undefined' && filterSettings.map) {
                ...
                if (!filterSettings.map[currentMapLayerName]) filterSettings.map[currentMapLayerName] = {};
                ...
                // If date.visuals is present, it represents the state at save time.
                // We should respect it for the current layer.
            ...
            // Note: Sliders updated via onchange or separate mechanism? 
            // If sliders exist, update them. FIX: Do NOT multiply by 100 (Active Tuning uses floats)
            const safeVal = (id, val) => { const el = document.getElementById(id); if (el) el.value = val; };
            // Use sanitized values from 't' (current layer) for the initial slider set
            safeVal('sld-map-br', filterSettings.map[currentMapLayerName].br);
            safeVal('sld-map-con', filterSettings.map[currentMapLayerName].con);
            safeVal('sld-map-op', filterSettings.map[currentMapLayerName].op);

            // Force UI update to reflect values
            updateSliderUI('map', filterSettings.map[currentMapLayerName]);
        }
